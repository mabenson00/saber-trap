(function () {
    ...
    class SFX {
        ...
        init() {
            if (this.ctx) return;
            try { this.ctx = new (window.AudioContext || window.webkitAudioContext)(); } catch (_) { /* silent */ }
        }
        ...
        wallPlace()  { this._t(400, .12, 'square', .08); this._t(600, .08, 'sine', .10, .03); }
        bounce()     { this._t(250 + Math.random() * 150, .04, 'sine', .05); }
        slash()      { this._t(150, .25, 'sawtooth', .07); this._t(300, .12, 'sine', .05); }
        gameOver()   { this._t(200, .4, 'sawtooth', .18); this._t(120, .55, 'sawtooth', .12, .15); }
        bank()       { this._t(500, .1, 'sine', .12); this._t(700, .1, 'sine', .12, .1); this._t(900, .14, 'sine', .12, .2); }
        puGood()     { this._t(600, .08, 'sine', .1); this._t(900, .12, 'sine', .1, .07); }
        puBad()      { this._t(200, .14, 'square', .1); this._t(100, .2, 'sawtooth', .07, .04); }
        claim()      { this._t(800, .14, 'sine', .05); this._t(1000, .1, 'sine', .04, .06); }
    }
    ...
    class Particles {
        ...
        burst(x, y, n, col, spd, life) {
            spd = spd || 200; life = life || .6;
            for (let i = 0; i < n; i++) {
                const a = Math.random() * Math.PI * 2;
                const s = spd * (.3 + Math.random() * .7);
                this.p.push({ x, y, vx: Math.cos(a) * s, vy: Math.sin(a) * s,
                    life: life * (.5 + Math.random() * .5), ml: life, col, sz: 1.5 + Math.random() * 2.5 });
            }
        }
        line(x1, y1, x2, y2, n, col) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                this.p.push({ x: lerp(x1, x2, t), y: lerp(y1, y2, t),
                    vx: nx * side * sp, vy: ny * side * sp,
                    life: .25 + Math.random() * .3, ml: .55, col, sz: 1 + Math.random() * 2 });
            }
        }
        update(dt) {
    ...
    class Game {
        ...
        resize() {
            ...
            if (wa > asp) { this.cvs.style.height = '100vh'; this.cvs.style.width = 'auto'; }
            else { this.cvs.style.width = '100vw'; this.cvs.style.height = 'auto'; }
        }
        mpos(e) {
        ...
        _initBlank() {
            ...
            this.effects = {};
            this.notifs = [];
            this.saber = { ext: false, o: null, e1: null, e2: null, p1: 0, p2: 0, d1: 0, d2: 0, cd: 0, close: false, angle: 0 };
            this.claimedPct = 0;
            this.wallCount = 0;
            this.combo = { n: 0, t: 0 };
            this.maxCombo = 0;
        ...
        _input() {
            ...
            this.cvs.addEventListener('mousemove', e => { this.mouse = this.mpos(e); });
            this.cvs.addEventListener('mousedown', e => { e.preventDefault(); action(); });
            this.cvs.addEventListener('touchstart', e => {
                e.preventDefault();
                this.mouse = this.mpos(e.touches[0]);
                this.sfx.init(); action();
            }, { passive: false });
            ...
            this.cvs.addEventListener('wheel', e => { e.preventDefault(); this.angle += (e.deltaY > 0 ? .1 : -.1); }, { passive: false });
        }
        ...
        _start() {
            ...
            const a = Math.random() * Math.PI * 2;
            this.balls = [{
                x: W / 2 + rand(-100, 100), y: H / 2 + rand(-60, 60),
                vx: Math.cos(a) * BALL_START_SPEED, vy: Math.sin(a) * BALL_START_SPEED,
                r: BALL_RADIUS, trail: [], frozen: false, fvx: 0, fvy: 0
            }];
            this.state = 'playing';
        }
        ...
        _flood(sx, sy, from, to) {
            ...
            const q = [sx + sy * GRID_W]; let qi = 0;
            this.grid[sy][sx] = to;
            while (qi < q.length) {
                const idx = q[qi++];
                const x = idx % GRID_W, y = (idx - x) / GRID_W;
        ...
        _claim() {
            ...
            for (let y = 0; y < GRID_H; y++) {
                for (let x = 0; x < GRID_W; x++) {
                    ...
                    else if (this.grid[y][x] === TEMP) { this.grid[y][x] = UNCLAIMED; unc++; }
            ...
            for (let y = 0; y < GRID_H; y++)
                for (let x = 0; x < GRID_W; x++)
                    if (this.grid[y][x] === UNCLAIMED) unc++;
        ...
        _rebuild() {
            for (let y = 0; y < GRID_H; y++)
                for (let x = 0; x < GRID_W; x++) this.grid[y][x] = UNCLAIMED;
        ...
        _updateBalls(dt) {
            ...
            for (const b of this.balls) {
                if (frozen) {
                    if (!b.frozen) { b.fvx = b.vx; b.fvy = b.vy; b.frozen = true; }
                    b.vx = 0; b.vy = 0; continue;
                }
                if (b.frozen) { b.vx = b.fvx; b.vy = b.fvy; b.frozen = false; }
                // normalise speed
                const spd = Math.sqrt(b.vx * b.vx + b.vy * b.vy);
                if (spd > .5) { const s = target / spd; b.vx *= s; b.vy *= s; }
        ...
        _ballSeg(b, s) {
            ...
            if (d < b.r && d > 0.001) {
                ...
                if (vd < 0) {
                    b.vx -= 2 * vd * nx; b.vy -= 2 * vd * ny;
                    // bounce randomness
                    const ra = (Math.random() - .5) * BALL_BOUNCE_RAND * 2;
                    const c = Math.cos(ra), sn = Math.sin(ra);
                    const nvx = b.vx * c - b.vy * sn;
                    const nvy = b.vx * sn + b.vy * c;
                    b.vx = nvx; b.vy = nvy;
                    this.sfx.bounce();
                    this.particles.burst(cp.x, cp.y, 3, '#00ffff', 80, .25);
                }
                b.x = cp.x + nx * b.r; b.y = cp.y + ny * b.r;
                return true;
            }
            return false;
        }
        ...
        _updateSaber(dt) {
            ...
            // current endpoints
            const cx = sb.o.x, cy = sb.o.y;
            const ex1 = lerp(cx, sb.e1.x, sb.p1), ey1 = lerp(cy, sb.e1.y, sb.p1);
            const ex2 = lerp(cx, sb.e2.x, sb.p2), ey2 = lerp(cy, sb.e2.y, sb.p2);
            ...
            for (const b of this.balls) {
                if (segVsCircle(ex2, ey2, ex1, ey1, b.x, b.y, b.r)) {
                    if (this.effects.ghost) {
                        ...
                        this._notif('GHOST BLADE!', '#88ff00'); continue;
                    }
                    if (this.effects.shield) {
                        delete this.effects.shield;
                        this._notif('SHIELD ABSORBED!', '#ffff00');
                        sb.ext = false; sb.cd = performance.now() + SABER_COOLDOWN; return;
                    }
                    this._die(b); return;
                }
            }
        ...
        _slash() {
            ...
            if (!this._isUnc(this.mouse.x, this.mouse.y)) {
                this._notif('MOVE TO UNCLAIMED AREA', '#ff3333'); return;
            }
        ...
        _completeWall() {
            ...
            if (now - this.combo.t < COMBO_WINDOW && this.combo.n > 0) this.combo.n++; else this.combo.n = 1;
            this.combo.t = now;
            this.maxCombo = Math.max(this.maxCombo, this.combo.n);
            if (sb.close) { this.closeCalls++; this._notif('CLOSE CALL! +30', '#ff00ff'); }

            this._calcScore();
            this.sfx.wallPlace(); this.sfx.claim();
            this.particles.line(sb.e2.x, sb.e2.y, sb.e1.x, sb.e1.y, 22, '#00ffff');
            this.shake = { i: 3, t: .15 };

            sb.ext = false; sb.cd = now + SABER_COOLDOWN;
        ...
        _updatePU(dt) {
            ...
            if (this.puTimer >= this.nextPu) {
                this.puTimer = 0; this.nextPu = rand(POWERUP_SPAWN_MIN, POWERUP_SPAWN_MAX);
                this._spawnPU();
            }
            // tick effect timers
            for (const k of Object.keys(this.effects)) {
                ...
                if (ef.tl !== undefined) {
                    ef.tl -= dt * 1000;
                    if (ef.tl <= 0) { delete this.effects[k]; this._notif(k.toUpperCase() + ' EXPIRED', '#666'); }
                }
            }
        ...
        _randUnc() {
            for (let i = 0; i < 120; i++) {
                ...
                let ok = true;
                for (const b of this.balls) if (dst(x, y, b.x, b.y) < 55) { ok = false; break; }
                if (ok) return { x, y };
            }
            return null;
        }
        ...
        _activatePU(p) {
            const d = p.def;
            if (d.pos) { this.sfx.puGood(); } else { this.sfx.puBad(); }
            this.particles.burst(p.x, p.y, 14, d.color, 140, .45);
            this._notif(d.name + (d.pos ? '!' : '!!'), d.color);

            switch (p.type) {
                case 'slowmo':  this.effects.slowmo = { tl: d.dur }; break;
                case 'freeze':  this.effects.freeze = { tl: d.dur }; break;
                case 'ghost':   this.effects.ghost = { uses: 1 }; break;
                case 'shield':  this.effects.shield = { uses: 1 }; break;
                case 'speed':   this.effects.speed = { tl: d.dur }; break;
                case 'phantom': this.effects.phantom = { tl: d.dur }; break;
                case 'multiball': {
                    const a = Math.random() * Math.PI * 2;
                    const sp = BALL_START_SPEED + (this.elapsed / 15) * BALL_SPEED_INC;
                    const pos = this._randUnc();
                    if (pos) this.balls.push({ x: pos.x, y: pos.y, vx: Math.cos(a) * sp, vy: Math.sin(a) * sp,
                        r: BALL_RADIUS, trail: [], frozen: false, fvx: 0, fvy: 0 });
                    break;
                }
                case 'wallrot':
        ...
        _die(ball) {
            ...
            this.sfx.gameOver();
            this.shake = { i: 14, t: .5 };
            this.particles.burst(ball.x, ball.y, 45, '#ff3300', 300, .8);
            this.particles.burst(ball.x, ball.y, 25, '#ffcc00', 200, .6);
            this._saveBests();
        }
        ...
        _stars() {
            ...
            if (this.claimedPct >= .60) return 1;
            return 0;
        }
        ...
        _render() {
            ...
            // screen shake
            if (this.shake.t > 0) {
                const int = this.shake.i * clamp(this.shake.t / .3, 0, 1);
                c.translate((Math.random() - .5) * int * 2, (Math.random() - .5) * int * 2);
            }
            ...
            if (this.walls.length > 0 || this.state !== 'menu') {
                ...
                c.strokeStyle = '#00ccdd'; c.lineWidth = 2;
                for (const w of this.walls) { c.beginPath(); c.moveTo(w.x1, w.y1); c.lineTo(w.x2, w.y2); c.stroke(); }
                c.restore();
                // core
                c.strokeStyle = 'rgba(180,255,255,.7)'; c.lineWidth = 1;
                for (const w of this.walls) { c.beginPath(); c.moveTo(w.x1, w.y1); c.lineTo(w.x2, w.y2); c.stroke(); }
            }

            // boundary glow
            c.save(); c.shadowColor = '#00aacc'; c.shadowBlur = 12;
            c.strokeStyle = '#00889966'; c.lineWidth = 2;
            ...
            for (const p of this.powerups) {
                ...
                c.save(); c.shadowColor = d.color; c.shadowBlur = 14;
                c.beginPath(); c.arc(p.x, p.y, r, 0, Math.PI * 2);
                c.fillStyle = d.glow; c.fill();
                ...
                c.restore();
                if (!d.pos) { c.strokeStyle = 'rgba(255,0,0,.25)'; c.lineWidth = 1; c.beginPath(); c.arc(p.x, p.y, r + 5, 0, Math.PI * 2); c.stroke(); }
            }
            ...
            for (const b of drawBalls) {
                ...
                for (let i = 0; i < b.trail.length; i++) {
                    ...
                    c.beginPath(); c.arc(b.trail[i].x, b.trail[i].y, sz, 0, Math.PI * 2);
                    c.fillStyle = `rgba(255,150,0,${al})`; c.fill();
                }
                // glow
                const gr = c.createRadialGradient(b.x, b.y, 0, b.x, b.y, b.r * 3);
                gr.addColorStop(0, 'rgba(255,150,0,.35)');
                gr.addColorStop(.5, 'rgba(255,80,0,.08)');
                gr.addColorStop(1, 'rgba(255,50,0,0)');
                c.fillStyle = gr; c.beginPath(); c.arc(b.x, b.y, b.r * 3, 0, Math.PI * 2); c.fill();
                // body
                const bAlpha = this.effects.phantom ? .22 : 1;
                c.globalAlpha = bAlpha;
                c.save(); c.shadowColor = '#ff9900'; c.shadowBlur = 14;
                c.beginPath(); c.arc(b.x, b.y, b.r, 0, Math.PI * 2);
                c.fillStyle = '#ff9900'; c.fill(); c.restore();
                c.beginPath(); c.arc(b.x, b.y, b.r * .45, 0, Math.PI * 2);
                c.fillStyle = '#ffffc8'; c.fill();
                c.globalAlpha = 1;
            }
            ...
            if (this.state === 'playing') {
                ...
                if (sb.ext) {
                    ...
                    const ex1 = lerp(cx, sb.e1.x, sb.p1), ey1 = lerp(cy, sb.e1.y, sb.p1);
                    const ex2 = lerp(cx, sb.e2.x, sb.p2), ey2 = lerp(cy, sb.e2.y, sb.p2);
                    c.save(); c.shadowColor = '#00ffff'; c.shadowBlur = 22;
                    c.strokeStyle = 'rgba(0,255,255,.55)'; c.lineWidth = 6;
                    ...
                    const dx = Math.cos(ea2) * 20, dy = Math.sin(ea2) * 20;
                    c.beginPath(); c.moveTo(mx - dx, my - dy); c.lineTo(mx + dx, my + dy); c.stroke();
                    c.lineWidth = 1;
                    c.beginPath(); c.moveTo(mx - (-dy * .35), my - (dx * .35)); c.lineTo(mx + (-dy * .35), my + (dx * .35)); c.stroke();
                    c.restore();
                }
            }
        ...
        _drawHUD(c) {
            ...
            if (ek.length) {
                c.textAlign = 'center'; let y = 12;
                for (const k of ek) {
                    const ef = this.effects[k]; const d = PU[k]; if (!d) continue;
                    let txt = d.name;
                    if (ef.tl !== undefined) txt += ' ' + (ef.tl / 1000).toFixed(1) + 's';
                    c.fillStyle = d.color;
                    c.font = '13px "Orbitron","Courier New",monospace';
                    c.fillText(txt, W / 2, y); y += 19;
                }
            }
            ...
            c.textAlign = 'left';
            c.fillText('WALLS: ' + this.wallCount + '  |  TIME: ' + Math.floor(this.elapsed) + 's  |  BALLS: ' + this.balls.length, 14, H - 10);

            // controls hint
            if (this.elapsed < 12) {
                c.textAlign = 'center';
                c.fillStyle = `rgba(255,255,255,${clamp(1 - this.elapsed / 12, 0, .35)})`;
                c.font = '10px "Orbitron","Courier New",monospace';
        ...
        _drawMenu(c) {
            ...
            c.save(); c.shadowColor = '#00ffff'; c.shadowBlur = 35;
            c.fillStyle = '#00ffff';
        ...
        // Paused
        _drawPaused(c) {
            c.fillStyle = 'rgba(5,5,18,.65)'; c.fillRect(0, 0, W, H);
            c.save(); c.shadowColor = '#00ffff'; c.shadowBlur = 20;
            c.fillStyle = '#00ffff';
        ...
        // Game Over
        _drawGO(c) {
            c.fillStyle = 'rgba(18,0,0,.78)'; c.fillRect(0, 0, W, H);
            c.save(); c.shadowColor = '#ff3300'; c.shadowBlur = 28;
            c.fillStyle = '#ff3300';
        ...
        // Banked
        _drawBanked(c) {
            c.fillStyle = 'rgba(0,12,22,.82)'; c.fillRect(0, 0, W, H);
            c.save(); c.shadowColor = '#00ff88'; c.shadowBlur = 28;
            c.fillStyle = '#00ff88';
            c.font = 'bold 44px "Orbitron","Courier New",monospace';
            c.textAlign = 'center'; c.textBaseline = 'middle';
        ...
        _scoreCard(c, sy) {
            ...
            c.font = '26px serif'; c.fillStyle = '#ffcc00';
            let st = '';
            ...
            c.fillText('Walls: ' + this.wallCount + '  |  Time: ' + Math.floor(this.elapsed) + 's  |  Best Combo: x' + this.maxCombo + '  |  Close Calls: ' + this.closeCalls, W / 2, sy + 62);

            c.save(); c.shadowColor = '#fff'; c.shadowBlur = 10;
            c.fillStyle = '#fff';
        ...
        // =====================================================================
        _frame(t) {
            const dt = Math.min((t - this._lt) / 1000, .05);
            this._lt = t;
            this._update(dt);
            this._render();
            requestAnimationFrame(this._frame);
